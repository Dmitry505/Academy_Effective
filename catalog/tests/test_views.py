...
class AuthorIntegrationTest(TestCase):
    '''appearance of the author in the list after adding'''
    def setUp(self):
        self.user = User.objects.create_user(username="librarian", password="testpass")
        permission = Permission.objects.get(codename="can_add_author")
        self.user.user_permissions.add(permission)
        self.client.login(username="librarian", password="testpass")

    def test_author_creation_flow(self):
        create_url = reverse("author_create")
        ...
        list_url = reverse("authors")
        response = self.client.get(list_url)

        expected_name = "Leo, Tolstoy"
        expected_dates = "( Sept. 9, 1828 - None )"
        self.assertContains(response, expected_name)
        self.assertContains(response, expected_dates)